 ...
 * @since 1.0.3
 */
class Pure_Top_Rated_Product_Widget extends Pure_Wc_Base_Widget {
	/**
	 * Get widget name.
	 *
	...
	public function get_title() {
		return esc_html__( 'Sidebar Top Rated', 'shopbuild' );
	}
    ...
    protected function register_content_controls(){
		...
		$this->add_control(
			'sb_price_rating_switch',
			[
			   'label'        => esc_html__( 'Price Rating Switch', 'shopbuild' ),
		...
		$this->add_control(
		 'sidebar_products_title_word_count',
		 [
		...
		if ( $r->have_posts() ) {
			...
			while ( $r->have_posts() ) {
                ...
                <li>
					...
					<?php do_action( 'woocommerce_widget_product_item_start', $template_args ); ?>
					...
					<div class="sb-sidebar-product-rating-item">
						...
						<div class="sb-sidebar-product-rating-content">
							<?php if ( $settings['sb_price_rating_switch'] == 'yes') : ?>
							...
							<h4 class="sb-sidebar-product-rating-title">
								<a class="tp-el-box-title" href="<?php echo esc_url( $product->get_permalink() ); ?>">
									<?php echo wp_kses(wp_trim_words($product->get_name(), $settings['sidebar_products_title_word_count']), pure_wc_get_kses_extended_ruleset()); ?>
								</a>
							</h4>
                    ...
                    <?php do_action( 'woocommerce_widget_product_item_end', $template_args ); ?>
                </li>
			<?php }
...
$widgets_manager->register( new Pure_Top_Rated_Product_Widget() );